...
import {
  checkIfFileExist,
  sizeOfExistingImage
} from '../../utilities/existingFileChecker';
import fs from 'fs';
...
images.get('/', (req: express.Request, res: express.Response) => {
  ...
  const fsPathThumbsFile = path.join(
    __dirname,
    '..',
    '..',
    '..',
    'images/thumb/' + filename + '-' + imageWidth + '-' + imageHeight + '.jpg'
  );
  ...
  if (
    checkIfFileExist(fsPathThumbsFile) &&
    sizeOfExistingImage(fsPathThumbsFile, imageWidth, imageHeight)
  ) {
